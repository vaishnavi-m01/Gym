...
const NewMember = () => {
    ...
    const validateField = (field: string, value: string) => {
        ...
        switch (field) {
            case "name":
                if (!value.match(/^[A-Za-z ]+$/)) errorMessage = "Invalid Name (Only letters allowed)";
                break;
            case "phone":
                if (!value.match(/^\d{10}$/)) errorMessage = "Phone number must be 10 digits";
                break;
            case "email":
                if (!value.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) errorMessage = "Invalid Email Format";
                break;
            case "dob":
                if (!value.match(/^\d{4}-\d{2}-\d{2}$/)) errorMessage = "Date must be in YYYY-MM-DD format";
                break;
            case "address":
                if (value.length < 10) errorMessage = "Address must be at least 10 characters";
                break;
            case "password":
                ...
                <View style={styles.subContainers}>
                    ...
                    <View style={styles.phoneContainer}>
                        ...
                        <View style={styles.phoneinputContainer}>
                            <TextInput
                                style={styles.inputbox}
                                placeholder={`${selectedCode} 9895xxxxxx`}  // Dynamically display the selected code
                                value={phone}
                                onChangeText={(text) => {
                                    setPhone(text);
                    ...
                    <View style={styles.textArea}>
                        ...
                        <Text style={styles.notesText}>4.  You can add any other notes here.</Text>
                    </View>
...
const styles = StyleSheet.create({
    ...
    pickerStyle: {
        width: "100%",
        color: "#0000",  // Ensures text is visible
    },
    Numbertext: {
        fontFamily: "Jost",
        fontWeight: 700,
